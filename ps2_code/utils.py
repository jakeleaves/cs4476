...
def show_correspondence_circles(imgA, imgB, X1, Y1, X2, Y2):
    ...
    for x1, y1, x2, y2 in zip(X1, Y1, X2, Y2):
        cur_color = np.random.rand(3)
        green = (0, 1, 0)
        newImg = cv2.circle(newImg, (int(x1), int(y1)), 10, cur_color, -1, cv2.LINE_AA)
        newImg = cv2.circle(newImg, (int(x1), int(y1)), 10, green, 2, cv2.LINE_AA)
        newImg = cv2.circle(newImg, (int(x2+shiftX), int(y2)), 10, cur_color, -1, cv2.LINE_AA)
        newImg = cv2.circle(newImg, (int(x2+shiftX), int(y2)), 10, green, 2, cv2.LINE_AA)
...
def show_correspondence_lines(imgA, imgB, X1, Y1, X2, Y2, line_colors=None):
    ...
    for x1, y1, x2, y2, dot_color, line_color in zip(X1, Y1, X2, Y2, dot_colors,
            line_colors):
        newImg = cv2.circle(newImg, (int(x1), int(y1)), 5, dot_color, -1)
        newImg = cv2.circle(newImg, (int(x2+shiftX), int(y2)), 5, dot_color, -1)
        newImg = cv2.line(newImg, (int(x1), int(y1)), (int(x2+shiftX), int(y2)), line_color, 2,
                                            cv2.LINE_AA)
    return newImg
